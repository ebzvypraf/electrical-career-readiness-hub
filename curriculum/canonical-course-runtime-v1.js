...
function evidenceProofSummary(id, context = {}) {
  const app = context.applicationEvidence || {};
  const check = context.assessmentResult || {};
  const history = Array.isArray(check.assessmentHistory) ? check.assessmentHistory : [];
  const priorEvidence = context.evidence;
  const applyReady = Boolean(app.tasksComplete && app.deliverable && app.decisions && app.assumptions && app.verification);
  const checkPassed = Boolean(check.passed || check.completionReady);
  const recovered = Boolean(check.recovered);
  const attempt = Number(check.attemptNumber || history.length || 1);
  const stale = Boolean(priorEvidence?.upstreamChangedAfterEvidence || priorEvidence?.invalidationReason);
  return { applyReady, checkPassed, recovered, attempt, stale };
}
