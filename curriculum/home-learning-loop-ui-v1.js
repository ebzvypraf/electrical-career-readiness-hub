/* Electrical Career Readiness Hub — Home learning-loop status v3.1. */
(function () {
  'use strict';
  ...
  function render() {
    ...
    const actionDraft=actionStage ? context.sessionDraft?.[actionStage] : null;
    const hasDraft=Boolean(actionDraft && !progress[actionStage]);
    ...
    const draftStamp=hasDraft ? String(actionDraft.savedAt||'draft') : '';
    const signature=`${weekId}|${statuses.map(x=>x.join(':')).join('|')}|${journal}|${portfolio}|${latestPassed}|${state.hubSignals?.overallProgress||0}|${integration.journalPrompt||''}|${integration.portfolioPrompt||''}|${next?.proofProgress||''}|${next?.source||''}|${next?.reason||''}|${actionStage||''}|${draftStamp}`;
    ...
    const actionLabel=latestPassed===false?'Resume recovery':hasDraft?`Resume ${LABELS[sessionStage]} draft`:`Start ${LABELS[sessionStage]} session`;
    const draftLine=hasDraft?`<div class="saved" style="margin-top:8px"><b>Saved draft ready to resume</b><small>Your unfinished ${LABELS[sessionStage]} work is already saved. Resume it instead of starting over.</small></div>`:'';
    panel.innerHTML=`<div class="k">Learning loop</div><h2>Week ${esc(weekId)} — ${esc(week.title||'Current learning module')}</h2><p class="muted">One connected path. Work only on the current stage, then continue from the canonical next action.</p><div class="summary">${STAGES.map((stage,i)=>`<div class="goal"><b>${i+1}. ${LABELS[stage]}</b><small><span class="pill ${statuses[i][0]==='complete'||statuses[i][0]==='ready'?'ok':''}">${esc(statuses[i][1])}</span>${stage===actionStage&&hasDraft?' <span class="tag">Draft saved</span>':''}</small></div>`).join('')}</div><div class="mission" style="margin-top:10px"><b>Next: ${esc(title)}</b><div class="muted">${esc(prompt)}</div><div class="muted" style="margin-top:7px"><b>Today’s focus:</b> ${esc(session.focus)}</div><div class="muted" style="margin-top:4px"><b>Typical session:</b> ${esc(session.minutes)} • <b>Done when:</b> ${esc(session.done)}</div><div class="muted" style="margin-top:7px"><b>Downstream proof:</b> ${esc(downstream)}</div>${next?.reason?`<div class="muted" style="margin-top:7px"><b>Why this is next:</b> ${esc(next.reason)}</div>`:''}${draftLine}${actionStage?`<button class="btn primary" id="home-learning-loop-open" style="margin-top:10px">${esc(actionLabel)}</button>`:''}</div><div class="learning-grid" style="margin-top:10px"><div class="learning-card"><h3>Journal reflection</h3><p class="muted">${esc(journalPrompt)}</p><button class="btn" id="home-learning-loop-journal">Reflect on this week</button></div><div class="learning-card"><h3>Portfolio evidence</h3><p class="muted">${esc(portfolioPrompt)}</p><button class="btn" id="home-learning-loop-portfolio">Open Portfolio</button></div></div>`;
    ...
    const journalBtn=document.getElementById('home-learning-loop-journal'); if(journalBtn)journalBtn.onclick=()=>{
      document.querySelector('[data-page="journal"]')?.click();
      setTimeout(()=>{
        const date=document.getElementById('jdate'), study=document.getElementById('jstudy'), learn=document.getElementById('jlearn'), nextField=document.getElementById('jnext');
        if(date&&!date.value)date.value=new Date().toISOString().slice(0,10);
        if(study&&!study.value)study.value=`Week ${weekId} — ${week.title||'Current learning module'} (${LABELS[sessionStage]}).`;
        if(learn&&!learn.value)learn.value=journalPrompt;
        if(nextField&&!nextField.value)nextField.value=`Continue with ${LABELS[sessionStage]} and complete the canonical definition of done: ${session.done}.`;
      },0);
    };
