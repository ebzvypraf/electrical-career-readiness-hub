 ...
 * v2.5 uses deterministic chronological Check ordering and normalized persisted
 * pass/fail values so Home cannot misread restored assessment state.
 */
(function () {
  'use strict';
  ...
  function normalizeHistory(history) {
    ...
    return records.sort((a, b) => {
      ...
      const aHasTime = Number.isFinite(aTime), bHasTime = Number.isFinite(bTime);
      if (aHasTime && bHasTime && aTime !== bTime) return aTime - bTime;
      if (aHasTime !== bHasTime) return aHasTime ? -1 : 1;
      const aAttempt = Number(a.item?.attemptNumber), bAttempt = Number(b.item?.attemptNumber);
      const aHasAttempt = Number.isFinite(aAttempt), bHasAttempt = Number.isFinite(bAttempt);
  ...
  function openNextStage(weekId, stage) {
    const id = String(weekId), stageName = String(stage || '').toLowerCase();
    if (!STAGES.includes(stageName)) return false;
    const canonicalSelector = `[data-canonical-open="${esc(id)}:${esc(stageName)}"]`, direct = document.querySelector(canonicalSelector);
    if (direct) { direct.click(); return true; }
    const stageIndex = STAGES.indexOf(stageName), legacy = document.querySelector(`[data-open="${Number(id) - 1}:${stageIndex}"]`);
    if (legacy) { legacy.click(); return true; }
    const target = Array.from(document.querySelectorAll('.week')).find(node => { const no = node.querySelector('.wno'); return no && Number((String(no.textContent).match(/\d+/) || [])[0]) === Number(id); });
    if (!target) return false;
    const buttons = Array.from(target.querySelectorAll('button'));
    const button = buttons.find(item => new RegExp('^\\s*' + stageName + '\\b', 'i').test(String(item.textContent || ''))) || buttons.find(item => String(item.textContent || '').toLowerCase().includes(stageName));
    if (button) { button.click(); return true; }
    return false;
  }
  function render() {
    const home = document.getElementById('home'), s = store();
    if (!home || !s?.getState) return;
    const state = s.getState(), weekId = activeWeek(state);
    if (!weekId) return;
    const week = api()?.catalog?.[weekId] || {}, context = state.contextByWeek?.[weekId] || {};
    let panel = document.getElementById('home-learning-loop');
    if (!panel) { panel = document.createElement('div'); panel.id = 'home-learning-loop'; panel.className = 'card s12'; const grid = home.querySelector('.grid'); if (!grid) return; grid.insertBefore(panel, grid.children[1] || null); }
    ...
    const checkHistory = normalizeHistory(context.assessmentHistory), latestCheck = checkHistory[checkHistory.length - 1] || context.assessmentResult || null;
    ...
    const journalCount = weekJournal.length, portfolioCount = weekPortfolio.length;
    const demonstrated = weekPortfolio.some(entry => entry?.demonstrated === true || entry?.reviewStatus === 'demonstrated');
    const remediation = context.remediation || {}, remediationConcepts = Array.isArray(remediation.concepts) ? remediation.concepts.filter(Boolean).slice(0, 3) : [];
