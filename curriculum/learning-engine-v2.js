...
export const CURRICULUM_URLS = {
  ...
  6: '/curriculum/week-06-asnzs-3000-application.json',
  7: '/curriculum/week-07-drawing-package-ownership.json',
  8: '/curriculum/week-08-pdb-layouts.json',
  9: '/curriculum/week-09-cable-sizing-voltage-drop.json',
  10: '/curriculum/week-10-protection-discrimination.json'
};
...
export function nextStage(progressByWeek, weekIds) {
  for (const id of weekIds || Object.keys(progressByWeek || {})) { const p = progressByWeek?.[id] || emptyProgress(); for (const stage of STAGES) if (!p[stage]) return { weekId: id, stage }; }
  return null;
}
export function isStageUnlocked(progressByWeek, weekId, stage) {
  const p = progressByWeek?.[weekId] || emptyProgress(), index = STAGES.indexOf(stage);
  if (index < 0) return false; if (index === 0) return true; return STAGES.slice(0, index).every(previous => Boolean(p[previous]));
}
...
export function deriveSignals(week, progress, context = {}) {
  ...
  return {
    completedStages: STAGES.filter(s => p[s]), completion: stageProgress(p),
    evidenceReady: Boolean(context.evidence?.title?.trim() && context.evidence?.description?.trim()),
    skills: week?.skills || [], journalPrompt: week?.integration?.journalPrompt || '',
    homeAction: week?.integration?.homeAction || '', portfolioPrompt: week?.integration?.portfolioPrompt || ''
  };
}
...
export function migrateLegacyProgress(legacyWeeks, weekIds = Object.keys(CURRICULUM_URLS)) {
  const ids = weekIds.map(String), source = Array.isArray(legacyWeeks) ? legacyWeeks : [], canonical = createLearningState(ids);
  ids.forEach((id, index) => { canonical[id] = { ...emptyProgress(), ...(source[index] || {}) }; }); return canonical;
}
export function mergeLearningProgress(current, incoming, weekIds = Object.keys(CURRICULUM_URLS)) {
  const ids = weekIds.map(String), base = createLearningState(ids);
  const a = Array.isArray(current) ? migrateLegacyProgress(current, ids) : (current || {}), b = Array.isArray(incoming) ? migrateLegacyProgress(incoming, ids) : (incoming || {});
  ids.forEach(id => { base[id] = { ...emptyProgress(), ...(a[id] || {}), ...(b[id] || {}) }; }); return base;
}
export function applyStageCompletion(progressByWeek, weekId, stage, context = {}) {
  ...
  if (!isStageUnlocked(progressByWeek, weekId, stage)) return { ok: false, progress: { ...current }, reason: 'Stage locked: complete the previous stage first' };
