...
export function buildAssessmentFeedback(questions = [], result = {}) {
  ...
  const reinforcement = failed.map(item => {
    ...
    return {
      questionId: clean(item?.id),
      prompt: clean(q?.prompt || q?.q),
      concepts,
      explanation: clean(q?.answer || q?.why) || 'Review the question, revisit the related learning material, and explain the reasoning in your own words.'
    };
  });
  return {
    engineVersion: ASSESSMENT_FEEDBACK_VERSION,
    status: failed.length ? 'reinforce' : (results.length ? 'ready' : 'not-assessed'),
    failedCount: failed.length,
    passedCount: results.filter(item => item.correct).length,
    reinforcement,
    priorityConcepts: [...new Set(reinforcement.flatMap(item => item.concepts))]
  };
}

export function shouldReinforce(result = {}) {
  return Boolean(result?.completionReady !== true || result?.passed !== true);
}
