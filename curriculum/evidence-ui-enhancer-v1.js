...
(function () {
  ...
  function enhance() {
    ...
    block.innerHTML = '<div class="learning-card"><h3>Evidence quality</h3>' +
      ...
      '</div>';
    form.insertBefore(block, document.getElementById('canon-save-evidence')); card.dataset.evidenceEnhanced = '1';
  }

  function capture(event) {
    const target = event.target && event.target.closest ? event.target.closest('#canon-save-evidence') : null; if (!target) return;
    const canonical = api(); const store = canonical && canonical.store; if (!store || typeof store.captureEvidence !== 'function') return;
    enhance(); const week = currentWeek(); if (!week) return;
    const titleEl = document.getElementById('canon-et'); const descriptionEl = document.getElementById('canon-ed'); const title = text(titleEl && titleEl.value); const description = text(descriptionEl && descriptionEl.value); if (!title || !description) return;
    ...
    const module = canonical.catalog?.[String(week)]; const criteria = Array.isArray(module?.evidence?.criteria) ? module.evidence.criteria : [];
