...
function contextForWeek(state, index) { const weekId = String(Number(index) + 1); const canonicalContext = store?.getState?.()?.contextByWeek?.[weekId] || {}; return { ...canonicalContext, applicationNotes: String(canonicalContext.applicationNotes ?? state.notes?.[index] ?? ''), applicationEvidence: canonicalContext.applicationEvidence || state.applicationEvidence?.[index] || null, assessmentResult: canonicalContext.assessmentResult || (state.checks?.[index] ? { ...state.checks[index], completionReady: Boolean(state.checks[index].completionReady ?? state.checks[index].passed) } : null), evidence: canonicalContext.evidence || state.evidence?.[index] || null }; }
