/* Electrical Career Readiness Hub — Evidence Engine v1.3.
 * Canonical Evidence -> Portfolio -> Skills -> Home/Journal contract.
 * Adds immutable proof-lineage metadata for evidence recapture/recovery.
 */
...
export function buildEvidenceLineage({ week, applyLink = '', checkLink = '', capturedAt = '', supersedesLineageId = '', recovery = false } = {}) {
  ...
  const apply = lineageToken(applyLink);
  const check = lineageToken(checkLink);
  const captured = lineageToken(capturedAt);
  const base = [normalizedWeek, apply, check, captured].filter(Boolean).join('|');
  return {
    lineageId: base ? `evidence:${base}` : '',
    week: normalizedWeek || null,
    applyLink: apply || null,
    checkLink: check || null,
    capturedAt: captured || null,
    supersedesLineageId: lineageToken(supersedesLineageId) || null,
...
function buildRecoveryProvenance(checkResult, context = {}) {
  ...
  const currentAttemptIndex = history.length ? history.findIndex(item => text(item?.date) === currentAttemptDate) : -1;
