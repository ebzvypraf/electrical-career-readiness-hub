 ...
 * existing entry point. The adapter is intentionally additive and safe to
 * remove because it is loaded as the final script in index.html.
 */
(function () {
  'use strict';
  ...
  function getNext() {
    ...
    return signals?.nextBestAction || null;
  }
  ...
  function weekStageFromModal() {
    ...
    return { week: Number(match[1]), stage: match[2].toLowerCase() };
  }
  ...
  function renderCanonicalModal() {
    ...
    if (stage === 'check') {
      ...
      const authored = questions.filter(q => Array.isArray(q.options) && Number.isInteger(q.correctIndex));
      if (authored.length) {
        ...
        body = `<div class="learning-hero"><b>Self-check</b><p>${escapeHtml(week.check?.passRule || 'Answer each question in your own words. The engine checks for required concepts; this is not a substitute for human review.')}</p></div>` +
    ...
    modalCard.innerHTML = `<div style="display:flex;justify-content:space-between;gap:10px"><div><div class="k">Week ${target.week} • ${stage.charAt(0).toUpperCase() + stage.slice(1)}</div><h2>${escapeHtml(week.title)}</h2><span class="pill">${escapeHtml(week.phase)}</span></div><button class="btn" id="canonical-close">Close</button></div>` +
      `<div class="mission" style="margin-top:12px"><b>${escapeHtml(stage.charAt(0).toUpperCase() + stage.slice(1))} stage</b><p class="muted">${escapeHtml(stageDescription(stage))}</p></div>` +
    ...
    document.getElementById('canonical-complete').onclick = () => {
      if (window.ECRH?.complete) window.ECRH.complete(target.week - 1, stageIndex(stage));
    };

    if (stage === 'apply') {
      document.getElementById('canonical-save-note').onclick = () => {
    ...
    if (stage === 'check') {
      document.getElementById('canonical-score').onclick = () => scoreCanonicalCheck(target.week, questions);
    }

    if (stage === 'evidence') {
      document.getElementById('canonical-save-evidence').onclick = () => {
        ...
        localStorage.setItem(STATE_KEY, JSON.stringify(current));
        if (window.ECRH?.setCheckResult) window.ECRH.setCheckResult(target.week - 1, current.checks?.[target.week - 1]);
        renderCanonicalModal();
  ...
  function scoreCanonicalCheck(weekNumber, questions) {
    ...
    state.checks[weekNumber - 1] = {
      score: result.score, total: result.total, passed: result.passed,
      percentage: result.percentage, engineVersion: result.engineVersion,
      gradingMode: result.gradingNote?.startsWith('Choice') ? 'choice' : 'self-check',
      date: new Date().toISOString()
    };
    localStorage.setItem(STATE_KEY, JSON.stringify(state));
    if (window.ECRH?.setCheckResult) window.ECRH.setCheckResult(weekNumber - 1, state.checks[weekNumber - 1]);
    renderCanonicalModal();
    refreshSurfaces(true);
  }

  function ensureSignalCard(id, containerId, title, initial) {
  ...
  function refreshSurfaces(force) {
    ...
    if (homeNext && next) homeNext.textContent = next.week;
    if (homeType && next) homeType.textContent = next.label;
    if (coach && next) coach.textContent = `${next.label}: ${next.week}`;
    if (coachText && next) coachText.textContent = next.prompt || stageDescription(next.stage);
    ...
    ensureSignalCard('canonical-home-signal', 'mission', 'Learning engine', `<b>Canonical 24-week engine</b><div class="muted">${signals.overallProgress}% complete • Next: ${escapeHtml(next ? `${next.week} — ${next.label}` : 'Program complete')}</div>`);

    const skills = document.getElementById('skills');
    if (skills) {
      skills.innerHTML = signals.skills.map(item => `<div class="skillrow"><div class="skillhead"><span>${escapeHtml(item.skill)}</span><b>${item.readiness}% readiness</b></div><div class="bar"><span style="width:${item.readiness}%"></span></div><small class="muted">${item.demonstratedWeeks}/${item.weeks} demonstrated • ${item.completedWeeks}/${item.weeks} fully completed</small></div>`).join('') || '<div class="empty">No canonical skill signals yet.</div>';
    }

    ensureSignalCard('canonical-skills-signal', 'advice', 'Skill signal', `<b>${signals.prioritySkillGaps.length ? 'Priority gaps identified' : 'No priority gaps'}</b><small>${signals.prioritySkillGaps.slice(0, 3).map(x => escapeHtml(`${x.skill} (${x.readiness}%)`)).join(' • ') || 'Keep progressing through the pathway.'}</small>`);

    ensureSignalCard('canonical-journal-signal', 'logs', 'Journal signal', `<b>${signals.journal.studyHours.toFixed(1)} study hours</b><small>${signals.journal.reflectionCount} reflections • Next action: ${escapeHtml(signals.journal.nextAction || 'Continue the next learning stage.')}</small>`);

    ensureSignalCard('canonical-portfolio-signal', 'readiness', 'Portfolio signal', `<b>${signals.portfolio.evidenceCount} evidence records</b><small>${signals.evidence.evidenceRate}% of weeks have evidence • Evidence-ready weeks: ${signals.evidence.evidenceReadyWeeks}/24</small>`);
  }

  async function load() {
    try {
      const [{ loadCanonicalCatalog }, assessment] = await Promise.all([
        import(CANONICAL_URL), import(ASSESSMENT_ENGINE_URL)
      ]);
      ...
      console.warn('Canonical learning UI adapter unavailable:', error);
    }
  }

  function installObserver() {
    const observer = new MutationObserver(() => {
      if (document.querySelector('#modal.show')) renderCanonicalModal();
