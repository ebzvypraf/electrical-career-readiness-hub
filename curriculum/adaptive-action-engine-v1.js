...
function evidenceIsStale(context = {}) {
  const evidence = context?.evidence;
  return Boolean(evidence && typeof evidence === 'object' && (
    evidence.upstreamChangedAfterEvidence === true ||
    evidence.reviewStatus === 'draft' ||
    evidence.invalidationReason ||
    context?.portfolio?.upstreamChangedAfterEvidence === true
  ));
}
...
function proofChainContext(context = {}) {
  ...
  if (!chain || typeof chain !== 'object') {
    return { hasProofChain: false, applyLinked: false, checkLinked: false, evidenceCaptured: false, demonstrated: false, stale: false };
  }
  const stale = evidenceIsStale(context);
  return {
    hasProofChain: true,
    applyLinked: Boolean(chain.apply?.linked),
    checkLinked: Boolean(chain.check?.linked),
    evidenceCaptured: Boolean(chain.evidence?.captured) && !stale,
    demonstrated: Boolean(chain.evidence?.demonstratedCapability) && !stale,
    stale
  };
}
...
function staleEvidenceCandidates(ids, catalog, progressByWeek, contextByWeek) {
  return ids.map(id => {
    ...
    if (!evidenceIsStale(context)) return null;
    const proof = proofChainStatus(context);
    const trail = assessmentTrail(context);
    const stage = proof.nextProofStage || (trail.latestPassed ? 'evidence' : 'check');
    if (!STAGES.includes(stage) || !catalog?.[id] || !isStageUnlocked(progressByWeek, id, stage)) return null;
    const reason = context?.evidence?.invalidationReason === 'new-check-attempt'
      ? 'A newer Check attempt invalidated the previous Evidence. Re-establish proof from the current Check result.'
      : 'Upstream Apply or Check work changed after Evidence was captured. Recover the proof chain before continuing.';
    const prompt = stage === 'evidence'
      ? 'Recapture Evidence from the current Apply and Check records; this submission will supersede the stale proof lineage.'
      : stage === 'check'
        ? 'Complete the current Check so the stale Evidence can be replaced with a valid proof record.'
        : 'Re-establish the current Apply deliverable before completing the Check and Evidence stages.';
    return {
      priority: 5,
      weekId: id,
      week: catalog[id]?.title || `Week ${id}`,
      stage,
      label: stage === 'evidence' ? 'Recover Evidence' : stage === 'check' ? 'Re-establish Check' : 'Re-establish Apply',
      prompt,
      reason,
      adaptive: true,
      source: 'stale-evidence-recovery',
      recovery: {
        required: true,
        staleEvidence: true,
        invalidationReason: text(context?.evidence?.invalidationReason) || 'upstream-changed',
        priorLineageId: text(context?.evidence?.lineage?.lineageId || context?.evidence?.supersedesLineageId) || null,
...
export function chooseNextBestAction({ catalog = {}, progressByWeek = {}, contextByWeek = {}, hubSignals = {} } = {}) {
  ...
  const remediationCandidates = ids.map(id => {
    ...
    return {
      ...
      prompt: remediation.status === 'ready-to-retry' ? 'Retry the Check after completing targeted reinforcement.' : `Reinforce: ${(remediation.concepts || []).join(', ') || 'the failed Check items'}.`,
  ...
  const staleCandidates = staleEvidenceCandidates(ids, catalog, progressByWeek, contextByWeek);
  if (staleCandidates.length) return staleCandidates[0];
  ...
  const gapCandidates = gaps.map((gap, index) => {
    ...
    const recoveryNote = trail.recovered ? ` This capability was recovered after ${trail.attempts} Check attempts; the next action preserves that recovery trail.` : '';
    const transferNote = transfer.transferReady ? ` ${transfer.demonstratedWeeks} prior week${transfer.demonstratedWeeks === 1 ? '' : 's'} already show strong evidence for this skill; use this stage to transfer it into a new context.` : '';
    const transferPrompt = transfer.transferReady ? `Transfer ${gap.skill} into the Week ${weekId} ${STAGE_LABELS[stage]} task and explicitly compare the new context with the prior demonstrated example.` : (gap.journalNextAction || `Strengthen ${gap.skill} through the ${STAGE_LABELS[stage]} stage.`);
    return {
      priority, weekId, week: catalog[weekId]?.title || gap.recommendedWeekTitle || `Week ${weekId}`, stage,
      label: transfer.transferReady ? `Transfer ${STAGE_LABELS[stage]}` : STAGE_LABELS[stage], prompt: transferPrompt,
      reason: `Priority competency gap: ${gap.skill} (${gap.readiness ?? 'developing'}% readiness).${recoveryNote}${transferNote}`,
      skill: gap.skill, adaptive: true, source: transfer.transferReady ? 'skill-gap-transfer' : 'skill-gap',
      assessmentTrail: { attempts: trail.attempts, recovered: trail.recovered, latestPassed: trail.latestPassed, latestPercentage: trail.latestPercentage },
      transfer: { transferReady: transfer.transferReady, demonstratedWeeks: transfer.demonstratedWeeks, evidenceQuality: transfer.evidenceQuality },
      evidenceQuality, proofChain: proof, ...actionContract(contextByWeek?.[weekId]), engineVersion: ADAPTIVE_ACTION_ENGINE_VERSION
    };
  }).filter(Boolean).sort((a, b) => a.priority - b.priority || Number(a.weekId) - Number(b.weekId));
  if (gapCandidates.length) return gapCandidates[0];

  const next = hubSignals?.nextBestAction || null;
  if (next?.weekId && next?.stage) return { ...next, adaptive: false, source: 'sequential', proofChain: proofChainStatus(contextByWeek?.[next.weekId]), ...actionContract(contextByWeek?.[next.weekId]), engineVersion: ADAPTIVE_ACTION_ENGINE_VERSION };
  for (const id of ids) {
    const progress = progressByWeek?.[id] || {};
    const stage = STAGES.find(candidate => !progress[candidate]);
    if (stage && isStageUnlocked(progressByWeek, id, stage)) return { weekId: id, week: catalog[id]?.title || `Week ${id}`, stage, label: STAGE_LABELS[stage], prompt: '', reason: 'Continue the canonical course sequence.', adaptive: false, source: 'sequential', proofChain: proofChainStatus(contextByWeek?.[id]), ...actionContract(contextByWeek?.[id]), engineVersion: ADAPTIVE_ACTION_ENGINE_VERSION };
  }
  return null;
}
